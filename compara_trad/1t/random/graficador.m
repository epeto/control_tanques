...
figure(1)
plot(altura_pid(1,:), altura_pid(2,:)*100, altura_pid(1,:), altura_pid(3,:)*100, altura_expert(1,:), altura_expert(3,:)*100, ...
    altura_fuzzy(1,:), altura_fuzzy(3,:)*100, 'linewidth', 1);
grid on;
xlabel('tiempo (s)');
ylabel('nivel (cm)');
legend('Ref', 'PID', 'Experto', 'Borroso');
