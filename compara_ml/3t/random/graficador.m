...
plot(altura_predictive(1,:), altura_predictive(2,:)*100, altura_predictive(1,:), altura_predictive(4,:)*100, ...
    altura_rl(1,:), altura_rl(4,:)*100, 'linewidth', 1);
grid on;
xlabel('tiempo (s)');
ylabel('nivel (cm)');
...
plot(altura_predictive(1,:), altura_predictive(3,:)*100, altura_predictive(1,:), altura_predictive(6,:)*100, ...
    altura_rl(1,:), altura_rl(6,:)*100, 'linewidth', 1);
grid on;
xlabel('tiempo (s)');
ylabel('nivel (cm)');
legend('Referencia', 'Predictivo', 'Refuerzo');
...
figure(6)
plot(flujo_predictive(1,:), flujo_predictive(3,:)*1000, flujo_rl(1,:), flujo_rl(3,:)*1000, 'linewidth', 1);
grid on;
xlabel('tiempo (s)');
ylabel('flujo (mL/s)');
legend('Predictivo', 'Refuerzo');
